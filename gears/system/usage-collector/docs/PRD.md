...
- [ ] Read paths return, unstripped, exactly the field set enumerated in `cpt-cf-usage-collector-fr-billing-fields-on-read` — that FR is the single normative enumeration — and point lookup by record `id` returns the exact persisted fact (cross-reference `cpt-cf-usage-collector-fr-billing-fields-on-read`)
