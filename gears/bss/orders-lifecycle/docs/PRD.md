...
| Order category: `new_sale` (this phase) / `change` (declared, phased — a line will target an existing subscription) | `p1` | Boundary = commercially initiated transactions (§1.1); `change` accepted for increases per the Change Orders PRD, other change kinds rejected until their path ships |
...
- **Add-on selection on the order line** → specified by the Change Orders PRD (§17), which owns the line-level add-on selection field for both moments it applies to — acquisition capture and a change line — and the add-on rule bounds the submit gate evaluates against it. This PRD does not author that field.
